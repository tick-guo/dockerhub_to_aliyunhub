...
fun_update(){
    ...
    # 没有合适的id比较， 只能比较时间了。
    org_digest=$(skopeo inspect --tls-verify=false docker://docker.io/$IMAGE:$TAG | jq -r '.Created')
    ali_digest=$(skopeo inspect --tls-verify=false docker://$REGISTRY/$NAMESPACE/$NEWIMAGE:$NEWTAG | jq -r '.Created')
...
do_main(){
    ...
    fun_update  node              18-buster-slim
    #
